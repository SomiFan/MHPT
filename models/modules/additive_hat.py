"""
additive_hat.py 2023/3/9 14:34
...
class SPWAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, head_dim, window_size, qkv_bias=True, qk_scale=None):
        ...
        self.n_tokens = window_size[0] * window_size[1]

        # define a parameter table of relative position bias
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH

        coords_h = torch.arange(self.window_size[0])  # 返回一个从0到window_size[0]（不包括window size）,步长为1的一维tensor
    ...
    def forward(self, x, y):
        ...
        #attn = torch.randn(256, 2, 64, 64).cuda()
...
class AHATBlock(nn.Module):
    """
    Additive Hybrid transformer block with shifted window scheme and cross-modal skip connection
    """
...
if __name__ == '__main__':
    ...
    head_dim = 30
    latent_dim = 128
    N = AHATBlock(input_resolution=to_2tuple(img_size), dim=embed_dim*n_scale, num_heads=num_heads,
                head_dim=head_dim, window_size=window_size,
                shift_size=0 , qkv_bias=qkv_bias,
                mlp_ratio=mlp_ratio, norm_layer=norm_layer,).cuda()
    from thop import profile
    from thop import clever_format

    u = torch.randn(1, 16384, 60).cuda()
    v = torch.randn(1, 16384, 60).cuda()
    macs, _ = profile(N, inputs=(u, v))
    ...
    flops = N.flops()
    print(f"number of Macs: {flops / 1e9}")